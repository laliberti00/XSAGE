...
// profiles
const sinkThr=1.5*(D.profiles.reduce((a,x)=>a+x.lens_KL,0)/D.profiles.length); // pozzo = KL >= 1.5×media (regola relativa del repo: battery_bfull.py:86, overnight_selection.py:58)
document.getElementById('profiles').innerHTML=D.profiles.map(s=>{
 const sink=s.lens_KL>=sinkThr;const fav=s.fav_genres.slice(0,3);const mx=Math.max(...fav.map(g=>g[1]));
